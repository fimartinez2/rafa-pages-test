      ...
      <Router>
        ...
        <Layout >
          <Layout style={{ height: "100%" }}>
            <Sider className="mobileHidden" width={211} style={{ height: "102%" }}>
              <Menu
                mode="inline"
              ...
              >
                ...
                {user != 'cesfamCoordinators' &&
                  <div>
                    <h2 style={mystyle.title}>{`${userData.firstName} ${userData.lastName}`}</h2>
                    <p style={mystyle.rol}>{`${userData.role}`}</p>
                  </div>
                }
                {user === 'cesfamCoordinators' &&
                  <div>
                    <h1 className="text-profile-cesfam"
                      style={mystyle.title}
                    >{`${userData.name}`}</h1>
                ...
                {user !== 'cesfamCoordinators' &&
                  <Menu.Item key="1" icon={<UserOutlined style={{ color: '#51379c' }} />}>
                    <span className="menuText">Mi perfil</span>
                    <Link to="/profile" />
                  </Menu.Item>}

                {user !== 'patients' && user !== 'caregivers' && user !== 'cesfamCoordinators' &&
                  <Menu.Item key="2" icon={<SmileOutlined style={{ color: '#51379c' }} />}>
                    <span className="menuText">Tablero de Ayuda</span>
                    <Link to="/help-board" />
                  </Menu.Item>}

                {(user === 'patients' || user === 'volunteers' || user === 'caregivers') &&
                  <Menu.Item key="3" icon={<SmileOutlined style={{ color: '#51379c' }} />}>
                    <a href="/activities">Actividades</a>
                  </Menu.Item>}

                {user === 'coordinators' &&
                  <Menu.Item key="4" icon={<HeartOutlined style={{ color: '#51379c' }} />}>
                    <a href="/volunteers-coordinators">Voluntarios</a>
                  </Menu.Item>}

                {user === 'coordinators' &&
                  <Menu.Item key="13" icon={<HeartOutlined style={{ color: '#51379c' }} />}>
                    <a href="/patients-coordinators">Pacientes</a>
                  </Menu.Item>}

                {user === 'coordinators' &&
                  <Menu.Item key="5" icon={<UsergroupAddOutlined style={{ color: '#51379c' }} />}>
                    <a href="/volunteer-requests">Solicitudes Voluntario</a>
                  </Menu.Item>}

                {user === 'cesfamCoordinators' &&
                  <Menu.Item key="6" icon={<UsergroupAddOutlined style={{ color: '#51379c' }} />}>
                    <a href="/users-cesfam">Pacientes y Cuidadores</a>
                  </Menu.Item>}

                {user === 'cesfamCoordinators' &&
                  <Menu.Item key="7" icon={<UsergroupAddOutlined style={{ color: '#51379c' }} />}>
                    <a href="/register-requests">Solicitudes Registro</a>
                  </Menu.Item>}

                {user === 'volunteers' &&
                  <Menu.Item key="9" icon={<TeamOutlined style={{ color: '#51379c' }} />}>
                    <span className="menuText">Pacientes</span>
                    <Link to="/patients-volunteers" />
                  </Menu.Item>}

                {(user === 'patients' || user === 'caregivers') &&
                  <Menu.Item key="11" icon={<PlusCircleOutlined style={{ color: '#51379c' }} />}>
                    <span className="menuText">Solicitud de ayuda</span>
                    <Link to="/form-help" />
                  </Menu.Item>}

                {user === 'caregivers' &&
                  <Menu.Item key="12" icon={<FormOutlined style={{ color: '#51379c' }} />}>
                    <span className="menuText">Reportes</span>
                    <Link to="/reports" />
                  </Menu.Item>}

                <Menu.Item key="10" icon={<LogoutOutlined style={{ color: '#51379c' }} />}>
                  <Popconfirm
            ...
            <Layout>
              ...
              >
                ...
                <Route exact path="/patients-volunteers" component={PatientsVolunteer} />
                <Route exact path="/register-requests" component={RegisterRequests} />
                <Route exact path="/volunteer-requests" component={VolunteerRequests} />
                <Route exact path="/volunteers-coordinators" component={VolunteersCoordinator} />
                <Route path="/volunteers-coordinators/new-ban/:id" component={NewBanVolunteer} />
                <Route exact path="/activities" component={Activities} />
                <Route path="/patients-volunteers/:id/new-activity" component={NewActivity} />
                <Route path="/activities/:id/report" component={Report} />
                <Route exact path="/users-cesfam" component={UsersCesfam} />
                <Route path="/users-cesfam/new-ban/:id" component={NewBanUser} />
                <Route path="/activities/eval-activity/:id" component={EvalActivity} />
                <Route exact path="/reports" component={ReportsPatient} />
                <Route exact path="/patients-coordinators" component={PatientsCoordinator} />
